...
func (m *MariaDB) observeOperation(operation, resource, subResource string, duration time.Duration, err error, rowsAffected int64, metadata map[string]interface{}) {
	...
	if resource == "" {
		resource = m.cfg.Connection.DbName
	}
