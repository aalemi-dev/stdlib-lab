...
// MariaDBContainer represents a MariaDB container for testing
...
// setupMariaDBContainer sets up a MariaDB container for testing
func setupMariaDBContainer(ctx context.Context) (*MariaDBContainer, error) {
	...
	portBindings := nat.PortMap{
		"3306/tcp": []nat.PortBinding{{HostPort: portStr}},
	}

	// Define container request - using MariaDB image
	req := testcontainers.ContainerRequest{
		...
		WaitingFor: wait.ForLog("mariadbd: ready for connections").WithStartupTimeout(60 * time.Second),
	}
	...
	// Wait for MariaDB to be fully ready for connections
	fmt.Printf("Waiting for MariaDB to be ready on %s:%s...\n", host, portStr)
	err = waitForMariaDBReady(host, portStr, "testuser", "testpass", "testdb", 60*time.Second)
	if err != nil {
		_ = container.Terminate(ctx)
		return nil, fmt.Errorf("MariaDB container not ready: %w", err)
	}
	fmt.Printf("MariaDB is ready on %s:%s\n", host, portStr)
	...
	return &MariaDBContainer{
		Container:        container,
		ConnectionString: fmt.Sprintf("testuser:testpass@tcp(%s:%s)/testdb?charset=utf8mb4&parseTime=True&loc=Local", host, portStr),
		Config:           config,
		Host:             host,
		Port:             portStr,
	}, nil
...
// waitForMariaDBReady attempts to connect to MariaDB until it's ready or times out
func waitForMariaDBReady(host, port, user, password, dbname string, timeout time.Duration) error {
	// Use standard MySQL connection DSN
	...
	for {
		if time.Since(startTime) > timeout {
			return fmt.Errorf("timed out waiting for MariaDB to be ready after %s", timeout)
		}
...
// TestMariaDBWithFXModule tests the mariadb package using the existing FX module
func TestMariaDBWithFXModule(t *testing.T) {
	// Skip if running in short mode
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}

	// Setup MariaDB containerInstance
	ctx := context.Background()
	containerInstance, err := setupMariaDBContainer(ctx)
	...
	t.Logf("Using MariaDB on %s:%s", containerInstance.Host, containerInstance.Port)

	// Get the MariaDB instance to test it
	var mariadb *MariaDB
	...
	t.Run("CRUDOperations", func(t *testing.T) {
		...
		err := mariadb.Create(ctx, &user)
		assert.NoError(t, err)
		assert.Greater(t, user.ID, uint(0))

		// Find
		var users []TestUser
		err = mariadb.Find(ctx, &users, "age = ?", 30)
		assert.NoError(t, err)
		...
		err = mariadb.First(ctx, &retrievedUser, "name = ?", "John Doe")
		assert.NoError(t, err)
		...
		err = mariadb.Save(ctx, &retrievedUser)
		assert.NoError(t, err)

		// Verify update with First
		var updatedUser TestUser
		err = mariadb.First(ctx, &updatedUser, retrievedUser.ID)
		assert.NoError(t, err)
		assert.Equal(t, 31, updatedUser.Age)

		// UpdateWhere
		_, err = mariadb.UpdateWhere(ctx, &TestUser{}, map[string]interface{}{
			"Age": 32,
		}, "name = ?", "John Doe")
		assert.NoError(t, err)

		// Verify UpdateWhere
		err = mariadb.First(ctx, &updatedUser, "name = ?", "John Doe")
		assert.NoError(t, err)
		assert.Equal(t, 32, updatedUser.Age)

		// Update specific model
		_, err = mariadb.Update(ctx, &updatedUser, map[string]interface{}{
			"Age": 33,
		})
		assert.NoError(t, err)
		...
		// UpdateColumn
		_, err = mariadb.UpdateColumn(ctx, &updatedUser, "Age", 34)
		assert.NoError(t, err)
		...
		// UpdateColumns
		_, err = mariadb.UpdateColumns(ctx, &updatedUser, map[string]interface{}{
			"Age":   35,
		...
		// Count
		var count int64
		err = mariadb.Count(ctx, &TestUser{}, &count, "age > ?", 30)
		assert.NoError(t, err)
		assert.Equal(t, int64(1), count)

		// Delete
		_, err = mariadb.Delete(ctx, &TestUser{}, "name = ?", "John Doe")
		assert.NoError(t, err)

		// Verify deletion with Count
		err = mariadb.Count(ctx, &TestUser{}, &count, "name = ?", "John Doe")
		assert.NoError(t, err)
	...
	t.Run("ExecRawSQL", func(t *testing.T) {
		...
		var items []Item
		err = mariadb.DB().Raw(`SELECT name, value FROM test_items ORDER BY value`).Scan(&items).Error
		assert.NoError(t, err)
	...
	t.Run("ErrorHandling", func(t *testing.T) {
		...
		t.Run("DirectGORMErrorChecking", func(t *testing.T) {
			var user TestUser
			err := mariadb.First(ctx, &user, "name = ?", "NonExistentUser")
			assert.Error(t, err)
			...
			err = mariadb.Query(ctx).Where("name = ?", "NonExistentUser").First(&user)
			assert.Error(t, err)
		...
		t.Run("ConstraintViolations", func(t *testing.T) {
			...
			// Create a record
			_, err = mariadb.Exec(ctx, `INSERT INTO unique_test (email) VALUES ('test@example.com')`)
			assert.NoError(t, err)
...
// TestMariaDBConnectionFailureRecovery tests connection failure and recovery
func TestMariaDBConnectionFailureRecovery(t *testing.T) {
	// Skip if running in short mode
	if testing.Short() {
		t.Skip("Skipping integration test in short mode")
	}
	...
	mariadb.retryChanSignal <- fmt.Errorf("test connection error")

	// Give time for a reconnection attempt
	time.Sleep(100 * time.Millisecond)

	// Connection should still work since the containerInstance is running
	db = mariadb.DB() // Get the DB again in case it was updated
	err = db.Raw("SELECT 1").Scan(&result).Error
	assert.NoError(t, err)
...
func TestErrorHandling(t *testing.T) {
	...
	t.Run("TranslateError", func(t *testing.T) {
		// Test nil error
		assert.Equal(t, nil, mariadb.TranslateError(nil))

		// Test GORM errors
		assert.Equal(t, ErrRecordNotFound, mariadb.TranslateError(gorm.ErrRecordNotFound))
		assert.Equal(t, ErrDuplicateKey, mariadb.TranslateError(gorm.ErrDuplicatedKey))
		assert.Equal(t, ErrForeignKey, mariadb.TranslateError(gorm.ErrForeignKeyViolated))
		assert.Equal(t, ErrInvalidData, mariadb.TranslateError(gorm.ErrInvalidData))
		assert.Equal(t, ErrTransactionFailed, mariadb.TranslateError(gorm.ErrInvalidTransaction))
		assert.Equal(t, ErrUnsupportedOperation, mariadb.TranslateError(gorm.ErrNotImplemented))
		assert.Equal(t, ErrInvalidQuery, mariadb.TranslateError(gorm.ErrMissingWhereClause))
		assert.Equal(t, ErrUnsupportedOperation, mariadb.TranslateError(gorm.ErrUnsupportedRelation))
		assert.Equal(t, ErrConstraintViolation, mariadb.TranslateError(gorm.ErrPrimaryKeyRequired))
		assert.Equal(t, ErrInvalidData, mariadb.TranslateError(gorm.ErrModelValueRequired))
		assert.Equal(t, ErrColumnNotFound, mariadb.TranslateError(gorm.ErrInvalidField))
		assert.Equal(t, ErrInvalidData, mariadb.TranslateError(gorm.ErrEmptySlice))
		assert.Equal(t, ErrUnsupportedOperation, mariadb.TranslateError(gorm.ErrDryRunModeUnsupported))
		assert.Equal(t, ErrConfigurationError, mariadb.TranslateError(gorm.ErrInvalidDB))
		assert.Equal(t, ErrInvalidData, mariadb.TranslateError(gorm.ErrInvalidValue))
		assert.Equal(t, ErrDataTooLong, mariadb.TranslateError(gorm.ErrInvalidValueOfLength))

		// Test error message patterns
		connectionRefusedErr := fmt.Errorf("connection refused")
		assert.Equal(t, ErrConnectionFailed, mariadb.TranslateError(connectionRefusedErr))

		connectionResetErr := fmt.Errorf("connection reset by peer")
		assert.Equal(t, ErrConnectionLost, mariadb.TranslateError(connectionResetErr))

		tooManyConnectionsErr := fmt.Errorf("too many connections")
		assert.Equal(t, ErrTooManyConnections, mariadb.TranslateError(tooManyConnectionsErr))

		deadlockErr := fmt.Errorf("deadlock detected")
		assert.Equal(t, ErrDeadlock, mariadb.TranslateError(deadlockErr))

		lockTimeoutErr := fmt.Errorf("lock wait timeout exceeded")
		assert.Equal(t, ErrLockTimeout, mariadb.TranslateError(lockTimeoutErr))

		dataTooLongErr := fmt.Errorf("data too long for column")
		assert.Equal(t, ErrDataTooLong, mariadb.TranslateError(dataTooLongErr))

		invalidJSONErr := fmt.Errorf("invalid json format")
		assert.Equal(t, ErrInvalidJSON, mariadb.TranslateError(invalidJSONErr))

		divisionByZeroErr := fmt.Errorf("division by zero")
		assert.Equal(t, ErrDivisionByZero, mariadb.TranslateError(divisionByZeroErr))

		tableNotExistErr := fmt.Errorf("table 'testdb.users' doesn't exist")
		assert.Equal(t, ErrTableNotFound, mariadb.TranslateError(tableNotExistErr))

		columnNotExistErr := fmt.Errorf("unknown column 'invalid_column'")
		assert.Equal(t, ErrColumnNotFound, mariadb.TranslateError(columnNotExistErr))

		accessDeniedErr := fmt.Errorf("access denied for user")
		assert.Equal(t, ErrPermissionDenied, mariadb.TranslateError(accessDeniedErr))

		diskFullErr := fmt.Errorf("disk full")
		assert.Equal(t, ErrDiskFull, mariadb.TranslateError(diskFullErr))

		// Test custom error (should return unchanged)
		customErr := fmt.Errorf("custom error")
		assert.Equal(t, customErr, mariadb.TranslateError(customErr))
	})

	t.Run("GetErrorCategory", func(t *testing.T) {
		// Connection errors
		assert.Equal(t, CategoryConnection, mariadb.GetErrorCategory(ErrConnectionFailed))
		assert.Equal(t, CategoryConnection, mariadb.GetErrorCategory(ErrConnectionLost))
		assert.Equal(t, CategoryConnection, mariadb.GetErrorCategory(ErrTooManyConnections))

		// Query errors
		assert.Equal(t, CategoryQuery, mariadb.GetErrorCategory(ErrInvalidQuery))
		assert.Equal(t, CategoryQuery, mariadb.GetErrorCategory(ErrQueryTimeout))
		assert.Equal(t, CategoryQuery, mariadb.GetErrorCategory(ErrStatementTimeout))
		assert.Equal(t, CategoryQuery, mariadb.GetErrorCategory(ErrInvalidCursor))
		assert.Equal(t, CategoryQuery, mariadb.GetErrorCategory(ErrCursorNotFound))

		// Data errors
		assert.Equal(t, CategoryData, mariadb.GetErrorCategory(ErrInvalidData))
		assert.Equal(t, CategoryData, mariadb.GetErrorCategory(ErrDataTooLong))
		assert.Equal(t, CategoryData, mariadb.GetErrorCategory(ErrInvalidDataType))
		assert.Equal(t, CategoryData, mariadb.GetErrorCategory(ErrInvalidJSON))
		assert.Equal(t, CategoryData, mariadb.GetErrorCategory(ErrDivisionByZero))
		assert.Equal(t, CategoryData, mariadb.GetErrorCategory(ErrNumericOverflow))

		// Constraint errors
		assert.Equal(t, CategoryConstraint, mariadb.GetErrorCategory(ErrDuplicateKey))
		assert.Equal(t, CategoryConstraint, mariadb.GetErrorCategory(ErrForeignKey))
		assert.Equal(t, CategoryConstraint, mariadb.GetErrorCategory(ErrConstraintViolation))
		assert.Equal(t, CategoryConstraint, mariadb.GetErrorCategory(ErrCheckConstraintViolation))
		assert.Equal(t, CategoryConstraint, mariadb.GetErrorCategory(ErrNotNullViolation))

		// Permission errors
		assert.Equal(t, CategoryPermission, mariadb.GetErrorCategory(ErrPermissionDenied))
		assert.Equal(t, CategoryPermission, mariadb.GetErrorCategory(ErrInsufficientPrivileges))
		assert.Equal(t, CategoryPermission, mariadb.GetErrorCategory(ErrInvalidPassword))
		assert.Equal(t, CategoryPermission, mariadb.GetErrorCategory(ErrAccountLocked))

		// Transaction errors
		assert.Equal(t, CategoryTransaction, mariadb.GetErrorCategory(ErrTransactionFailed))
		assert.Equal(t, CategoryTransaction, mariadb.GetErrorCategory(ErrDeadlock))
		assert.Equal(t, CategoryTransaction, mariadb.GetErrorCategory(ErrSerializationFailure))
		assert.Equal(t, CategoryTransaction, mariadb.GetErrorCategory(ErrIdleInTransaction))

		// Resource errors
		assert.Equal(t, CategoryResource, mariadb.GetErrorCategory(ErrDiskFull))
		assert.Equal(t, CategoryResource, mariadb.GetErrorCategory(ErrLockTimeout))

		// System errors
		assert.Equal(t, CategorySystem, mariadb.GetErrorCategory(ErrInternalError))
		assert.Equal(t, CategorySystem, mariadb.GetErrorCategory(ErrSystemError))
		assert.Equal(t, CategorySystem, mariadb.GetErrorCategory(ErrIndexCorruption))
		assert.Equal(t, CategorySystem, mariadb.GetErrorCategory(ErrProtocolViolation))
		assert.Equal(t, CategorySystem, mariadb.GetErrorCategory(ErrConfigurationError))

		// Schema errors
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrTableNotFound))
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrColumnNotFound))
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrDatabaseNotFound))
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrSchemaNotFound))
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrFunctionNotFound))
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrTriggerNotFound))
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrIndexNotFound))
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrViewNotFound))
		assert.Equal(t, CategorySchema, mariadb.GetErrorCategory(ErrSequenceNotFound))

		// Operation errors
		assert.Equal(t, CategoryOperation, mariadb.GetErrorCategory(ErrUnsupportedOperation))
		assert.Equal(t, CategoryOperation, mariadb.GetErrorCategory(ErrMigrationFailed))
		assert.Equal(t, CategoryOperation, mariadb.GetErrorCategory(ErrBackupFailed))
		assert.Equal(t, CategoryOperation, mariadb.GetErrorCategory(ErrRestoreFailed))
		assert.Equal(t, CategoryOperation, mariadb.GetErrorCategory(ErrSchemaValidation))

		// Unknown errors
		customErr := fmt.Errorf("some unknown error")
		assert.Equal(t, CategoryUnknown, mariadb.GetErrorCategory(customErr))
	})
	...
	t.Run("IsRetryable", func(t *testing.T) {
		...
		for _, err := range retryableErrors {
			assert.True(t, mariadb.IsRetryable(err), "Error %v should be retryable", err)
		}
		...
		for _, err := range nonRetryableErrors {
			assert.False(t, mariadb.IsRetryable(err), "Error %v should not be retryable", err)
		}
	})
	...
	t.Run("IsTemporary", func(t *testing.T) {
		...
		for _, err := range temporaryErrors {
			assert.True(t, mariadb.IsTemporary(err), "Error %v should be temporary", err)
		}
		...
		for _, err := range nonTemporaryErrors {
			assert.False(t, mariadb.IsTemporary(err), "Error %v should not be temporary", err)
		}
	})
	...
	t.Run("IsCritical", func(t *testing.T) {
		...
		for _, err := range criticalErrors {
			assert.True(t, mariadb.IsCritical(err), "Error %v should be critical", err)
		}
		...
		for _, err := range nonCriticalErrors {
			assert.False(t, mariadb.IsCritical(err), "Error %v should not be critical", err)
		}
	})
	...
	t.Run("RealDatabaseErrors", func(t *testing.T) {
		...
		var user TestUser
		err = mariadb.First(ctx, &user, "id = ?", 99999)
		translatedErr := mariadb.TranslateError(err)
		...
		_, err = mariadb.Exec(ctx, "INSERT INTO test_constraints (email, name, age) VALUES (?, ?, ?)",
		...
		_, err = mariadb.Exec(ctx, "INSERT INTO test_constraints (email, age) VALUES (?, ?)",
			"test2@example.com", 25)
		translatedErr = mariadb.TranslateError(err)
		assert.Equal(t, ErrNotNullViolation, translatedErr)

		// Test check constraint violation (if supported by MariaDB version)
		_, err = mariadb.Exec(ctx, "INSERT INTO test_constraints (email, name, age) VALUES (?, ?, ?)",
		...
		_, err = mariadb.Exec(ctx, "SELECT * FROM non_existent_table")
		translatedErr = mariadb.TranslateError(err)
		assert.Equal(t, ErrTableNotFound, translatedErr)

		// Test column not found
		_, err = mariadb.Exec(ctx, "SELECT non_existent_column FROM test_constraints")
		translatedErr = mariadb.TranslateError(err)
		assert.Equal(t, ErrColumnNotFound, translatedErr)

		// Test invalid SQL syntax
		_, err = mariadb.Exec(ctx, "INVALID SQL STATEMENT")
		translatedErr = mariadb.TranslateError(err)
		assert.Equal(t, ErrInvalidQuery, translatedErr)

		// Clean up
		_, err = mariadb.Exec(ctx, "DROP TABLE IF EXISTS test_constraints")
		require.NoError(t, err)
		_, err = mariadb.Exec(ctx, "DROP TABLE IF EXISTS test_users")
		require.NoError(t, err)
	})
...
func TestErrorCategoriesComprehensive(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			category := mariadb.GetErrorCategory(tt.err)
...
func TestErrorClassificationFunctions(t *testing.T) {
	...
	t.Run("AllErrorsClassified", func(t *testing.T) {
		...
		for _, err := range allErrors {
			category := mariadb.GetErrorCategory(err)
...
func TestAdvancedQueryOperations(t *testing.T) {
	...
	err = mariadb.AutoMigrate(&Category{}, &Product{}, &Customer{}, &Order{}, &OrderItem{})
	require.NoError(t, err)

	// Seed test data
	err = seedTestData(ctx, mariadb)
	require.NoError(t, err)

	// Run subtests
	t.Run("ComplexWhereClause", testComplexWhereClauses(ctx, mariadb))
	t.Run("JoinsAndRelationships", testJoinsAndRelationships(ctx, mariadb))
	t.Run("GroupByHavingClauses", testGroupByHavingClauses(ctx, mariadb))
...
func seedTestData(ctx context.Context, mariadb *MariaDB) error {
	// Create categories
	categories := []Category{
		{Name: "Electronics"},
		{Name: "Clothing"},
	...
	for i := range categories {
		if err := mariadb.Create(ctx, &categories[i]); err != nil {
	...
	for i := range products {
		if err := mariadb.Create(ctx, &products[i]); err != nil {
	...
	for i := range customers {
		if err := mariadb.Create(ctx, &customers[i]); err != nil {
	...
	for i := range orders {
		if err := mariadb.Create(ctx, &orders[i]); err != nil {
	...
	for i := range orderItems {
		if err := mariadb.Create(ctx, &orderItems[i]); err != nil {
	...
	for _, order := range orders {
		var items []OrderItem
		if err := mariadb.Find(ctx, &items, "order_id = ?", order.ID); err != nil {
		...
		if _, err := mariadb.Update(ctx, &order, map[string]interface{}{
			"total_amount": total,
		}); err != nil {
...
func testJoinsAndRelationships(ctx context.Context, mariadb *MariaDB) func(t *testing.T) {
	return func(t *testing.T) {
		// Test 1: INNER JOIN
...
func testGroupByHavingClauses(ctx context.Context, mariadb *MariaDB) func(t *testing.T) {
	return func(t *testing.T) {
		...
		// Test 4: GROUP BY with date parts (MySQL syntax)
...
func TestTransactionHandling(t *testing.T) {
	...
	// Setup test accounts
	setupTestAccounts(t, ctx, mariadb)

	// Run subtests
	t.Run("BasicTransactionCommit", testBasicTransactionCommit(ctx, mariadb))
	t.Run("TransactionRollback", testTransactionRollback(ctx, mariadb))
...
func setupTestAccounts(t *testing.T, ctx context.Context, mariadb *MariaDB) {
	...
	for _, account := range accounts {
		err := mariadb.Create(ctx, &account)
		require.NoError(t, err)
...
func testBasicTransactionCommit(ctx context.Context, mariadb *MariaDB) func(t *testing.T) {
	return func(t *testing.T) {
		// Get initial balances
		var aliceAccount, bobAccount Account
		err := mariadb.First(ctx, &aliceAccount, "name = ?", "Alice")
		require.NoError(t, err)
		err = mariadb.First(ctx, &bobAccount, "name = ?", "Bob")
		require.NoError(t, err)
		...
		err = mariadb.First(ctx, &aliceAccount, aliceAccount.ID)
		require.NoError(t, err)
		err = mariadb.First(ctx, &bobAccount, bobAccount.ID)
		require.NoError(t, err)
		...
		err = mariadb.First(ctx, &txLog, "from_account_id = ? AND to_account_id = ?", aliceAccount.ID, bobAccount.ID)
		require.NoError(t, err)
...
func testTransactionRollback(ctx context.Context, mariadb *MariaDB) func(t *testing.T) {
	return func(t *testing.T) {
		// Get initial balances
		var charlieAccount, daveAccount Account
		err := mariadb.First(ctx, &charlieAccount, "name = ?", "Charlie")
		require.NoError(t, err)
		err = mariadb.First(ctx, &daveAccount, "name = ?", "Dave")
		require.NoError(t, err)
		...
		err = mariadb.First(ctx, &charlieAccount, charlieAccount.ID)
		require.NoError(t, err)
		err = mariadb.First(ctx, &daveAccount, daveAccount.ID)
		require.NoError(t, err)
		...
		var count int64
		err = mariadb.Count(ctx, &TransactionLog{}, &count, "from_account_id = ? AND to_account_id = ?", charlieAccount.ID, daveAccount.ID)
		require.NoError(t, err)
		assert.Equal(t, int64(0), count)
	}
}
...
func TestQueryBuilder_Create(t *testing.T) {
	...
	err = mariadb.DB().AutoMigrate(&TestUser{})
	require.NoError(t, err)
	...
	t.Run("Create_Simple", func(t *testing.T) {
		...
		rowsAffected, err := mariadb.Query(ctx).Create(&user)
		require.NoError(t, err)
		...
		err = mariadb.First(ctx, &fetchedUser, user.ID)
		require.NoError(t, err)
	...
	t.Run("Create_WithOnConflictDoNothing", func(t *testing.T) {
		...
		rowsAffected, err := mariadb.Query(ctx).
		...
		var count int64
		err = mariadb.Count(ctx, &TestUser{}, &count, "email = ?", "bob@unique.com")
		require.NoError(t, err)
		assert.Equal(t, int64(1), count)
	})
	...
	t.Run("Create_WithOnConflictUpdate", func(t *testing.T) {
		...
		var fetchedUser TestUser
		err = mariadb.First(ctx, &fetchedUser, "email = ?", "charlie@example.com")
		require.NoError(t, err)
...
func TestQueryBuilder_ToSubquery(t *testing.T) {
	...
	t.Run("ToSubquery_NotIN", func(t *testing.T) {
		...
		youngAgesSubquery := mariadb.Query(ctx).
			Model(&TestUser{}).
			Select("age").
			Where("age < ?", 25).
			ToSubquery()
	...
	t.Run("ToSubquery_MultipleSubqueries", func(t *testing.T) {
		...
		// Find minimum age
		minAgeSubquery := mariadb.Query(ctx).
			Model(&TestUser{}).
			Select("MIN(age)").
			ToSubquery()
		...
		// Find maximum age
		maxAgeSubquery := mariadb.Query(ctx).
			Model(&TestUser{}).
			Select("MAX(age)").
			ToSubquery()
